...
def api_endpoint():
    ...
    #设置最大组别
    global global_totalGroupNums
    global_totalGroupNums=totalGroupNums
...
def addNewStd():
   # GetData=request.json()
    ...
    addToGroup=request.json.get('toGroup')
    addedStdID=request.json.get('stdID')
    ...
    print('总组数',tmpGroupNum)
    print('添加 ',nameToAdd)
    print('到 ',addToGroup)

    #mod
    if addToGroup<=tmpGroupNum:
        supervising=(int(addToGroup))%tmpGroupNum+1
    else:
        return jsonify({"message":'组别不存在'})
    
    #print(supervising)

    '''newStd={
        "group id":0,  #这个group id是组内第几个，不是第几组
        "name":nameToAdd,  
        "student id":addedStdID,  
        "shoot": True,  
        "program": True,  
        "absent": False,
        "absent times":0,
        "supervised group":supervising,
    }'''

    #print(newStd)
    for key,value in data.items():
        if int(addToGroup)==int(key):
            tmpGroupid=-1
            #print(value)
            for tmpS in value:
                #print(tmpS)
            ...
            print(tmpGroupNum)
            data[key].append({
            "group id":tmpGroupNum,  #这个group id是组内第几个，不是第几组
            "name":nameToAdd,  
            "student id":addedStdID,  
            "shoot": True,  
            "program": True,  
